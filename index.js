...
client.on("messageCreate", (msg) => {
 if (msg.channel === client.config.channel && msg.author === client.config.owner) {
        ...
        const wait = new Discord.MessageEmbed()
         .setDescription("<a:loading:895227261752582154> Waiting for server response...")
            .setColor("#5865f2")
...
client.on("ready", async () => {
 ...
 if (!client.config.channel.guild.me.permissionsIn(client.config.channel).has("VIEW_CHANNEL")) {
  throw new Error("Missing required permission 'Read Messages' for channel specified in config.json");
 }

 if (!client.config.channel.guild.me.permissionsIn(client.config.channel).has("SEND_MESSAGES")) {
    throw new Error("Missing required permission 'Send Messages' for channel specified in config.json");
   }
  
   if (!client.config.channel.guild.me.permissionsIn(client.config.channel).has("MANAGE_WEBHOOKS")) {
    throw new Error("Missing required permission 'Manage Webhooks' for channel specified in config.json");
   }
